...
export default class RestService {
  constructor() {
    ...
    */
    this.GET = function(url, params, successFunction, errorFunction, lockScreen = true, timeout) {
      this.beforeSendQuery(lockScreen)
      this.queryExecuter(3, 'GET', url, params, undefined, successFunction, errorFunction, timeout)
    }

    this.POST = function(url, data, successFunction, errorFunction, lockScreen = true, timeout) {
      this.beforeSendQuery(lockScreen)
      this.queryExecuter(3, 'POST', url, undefined, data, successFunction, errorFunction, timeout)
    }

    this.PUT = function(url, data, successFunction, errorFunction, lockScreen = true, timeout) {
      this.beforeSendQuery(lockScreen)
      this.queryExecuter(3, 'PUT', url + '/' + data.ID, undefined, data, successFunction, errorFunction, timeout)
    }

    this.DELETE = function(url, data, successFunction, errorFunction, lockScreen = true, timeout) {
      this.beforeSendQuery(lockScreen)
      this.queryExecuter(3, 'DELETE', url + '/' + data.ID, undefined, data, successFunction, errorFunction, timeout)
    }

    this.EXECUTE_CONCURRENT = function(method, params, successFunction, errorFunction, lockScreen = true, timeout) {
      this.beforeSendQuery(lockScreen)
      this.runArrayOfPromises(method, 3, params, successFunction, errorFunction, timeout) // createPromises
    }

    this.EXECUTE_CONCURRENT_WITH_CUSTOM_METHODS = function(params, successFunction, errorFunction, lockScreen = true, timeout) {
      this.beforeSendQuery(lockScreen)
      this.runArrayOfPromises('custom', 3, params, successFunction, errorFunction, timeout) // createCustomPromises
    }
    ...
    this.createPromises = function(method, params, timeout, API) {
      ...
      for (let i = 0; i < params.length; i++) {
        ...
        promises.push(this.execute(method, url, method === 'GET' ? params[i].parameters : undefined, method === 'GET' ? undefined : params[i].parameters,
          timeout, API))
      }
      return promises
    }
    ...
    this.runArrayOfPromises = function(type, nOfRetries, params, successFunction, errorFunction, timeout) {
      let promises = []
      if (type === 'custom') {
        promises = this.createCustomPromises(params, timeout)
      } else { // then type= method type like GET, DEL etc.
        promises = this.createPromises(type, params, timeout)
      }
        ...
        if (nOfRetries >= -5 && typeof response !== 'undefined' && response.response !== undefined && response.response.status === 401) {
          store.dispatch({
            ...
            successCallback: function() {
              setTimeout(function() {
                instance.runArrayOfPromises(type, nOfRetries, params, successFunction, errorFunction, timeout)
              }, 2000)
            }
          })
        } else {
      ...
      if (nOfRetries >= 0) {
        axios.all(promises).then(latterSuccessFunction).catch(function(res) {
    ...
    this.execute = function(method, url, params, data, timeout = 15000, API) {
      var request = {
        _dc: new Date().getTime(),
        method: method,
        // url: 'http://kesfet.be/nserver/' + url,
        url: 'http://173.212.229.149:8448/' + url,
        params: params,
        data: data
      }
      if ((url.indexOf('/Update') > -1) || (url.indexOf('/Insert') > -1) || (url.indexOf('/Delete') > -1)) {
